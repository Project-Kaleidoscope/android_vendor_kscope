...
if __name__ == '__main__':
    # Default to Project Kaleidoscope Gerrit
    default_gerrit = 'https://review.kaleidoscope.ink'

    parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter, description=textwrap.dedent('''\
        repopick.py is a utility to simplify the process of cherry picking
        patches from Kaleidoscope's Gerrit instance (or any gerrit instance of your choosing)
    ...
    for item in mergables:
        ...
        if args.gerrit == default_gerrit:
            if args.verbose:
                print('Trying to fetch the change from our org')

            if args.pull:
                cmd = ['git pull --no-edit kscope', item['fetch'][method]['ref']]
            else:
                cmd = ['git fetch kscope', item['fetch'][method]['ref']]
            if args.quiet:
                cmd.append('--quiet')
            else:
                print(cmd)
        ...
        if args.gerrit != default_gerrit or os.stat(FETCH_HEAD).st_size == 0:
            # If not using the default gerrit or our org failed, fetch from gerrit.
            if args.verbose:
                if args.gerrit == default_gerrit:
                    print('Fetching from our org didn\'t work, trying to fetch the change from Gerrit')
                else:
