...
func init() {
	android.RegisterModuleType("kscope_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = kscopeExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = kscopeExpandVariables(ctx, dep_file)
	...
	cmd := kscopeExpandVariables(ctx, String(g.properties.Cmd))
